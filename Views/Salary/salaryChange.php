...
// 給与変更確認画面,
// 給与一覧画面から遷移
if(strstr($referer, $url) || strstr($referer, $salaryChangeConfirmUrl) || strstr($referer, $salaryListUrl)){
  ...
  if(strstr($referer, $url)){
    ...
    if(empty($errorMsg)){
      ...
      $_SESSION['salaryChange'] = $_POST;

      // 給与変更確認画面へ遷移
      header('Location: salaryChangeConfirm.php');
      exit;
    }

  // 給与変更確認画面から戻る押下
  } else if (!empty($_SESSION['salaryChange'])){
...
<html lang="ja">
  ...
  <body>
    ...
    <main>
      <!-- 給与変更フォーム -->
      <form action="salaryChange.php" method="post">
